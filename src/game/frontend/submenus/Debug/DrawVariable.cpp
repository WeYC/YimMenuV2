...
namespace YimMenu
{
	...
	case VariableType::INT:
	{
		...
		ImGui::InputScalar("##value_int", ImGuiDataType_S32, static_cast<int*>(value));
		break;
	}
	case VariableType::INT64:
	{
		ImGui::SetNextItemWidth(200.f);
		ImGui::InputScalar("##value_int64", ImGuiDataType_S64, static_cast<std::int64_t*>(value));
		break;
	}
	case VariableType::BITSET:
	{
		ImGui::SetNextItemWidth(200.f);
		ImGui::Bitfield("##value_bitset", static_cast<int*>(value));
		break;
	}
	case VariableType::BOOL:
	{
		ImGui::Checkbox("##value_bool", static_cast<bool*>(value));
		break;
	}
	case VariableType::FLOAT:
	{
		ImGui::SetNextItemWidth(200.f);
		ImGui::InputScalar("##value_float", ImGuiDataType_Float, static_cast<float*>(value));
		break;
	}
	...
	{
		ImGui::SetNextItemWidth(200.f);
		ImGui::InputText("##value_string", static_cast<char*>(value), 255);
		break;
	}
		}
	}
	...
	void DrawSavedVariablePreview(const SavedVariableBase& var, void* value)
	{
		if (!value)
	{
		ImGui::Selectable(L("generic.invalid", "Invalid").c_str(), false, ImGuiSelectableFlags_Disabled);
			return;
		}
	...
	void DrawSavedVariable(SavedVariableBase& var)
	{
		...
		ImGui::InputScalar("##var_index", ImGuiDataType_U32, &var.base, &step, &step_fast);
		ImGui::SameLine();
		...
		if (ImGui::Button(L("debug.variable.add_offset", "Add Offset").c_str()))
			var.appendages.push_back({VariableAppendage::Type::OFFSET, 0LL, 0ULL});
		ImGui::SameLine();
		if (ImGui::Button(L("debug.variable.add_read_player_id", "Add Read Player Id").c_str()))
			var.appendages.push_back({VariableAppendage::Type::PLAYER_ID, 0LL, 0ULL});

		if (var.appendages.size() > 0)
		{
			ImGui::SameLine();
			if (ImGui::Button(L("debug.variable.remove_offset", "Remove Offset").c_str()))
			{
		...
		ImGui::SameLine();
		if (ImGui::Button(L("generic.clear", "Clear").c_str()))
		{
			var.base = 0;
