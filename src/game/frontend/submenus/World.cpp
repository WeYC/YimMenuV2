...
namespace YimMenu::Submenus
{
	World::World() :
	    ...
	    Submenu::Submenu(L("submenu.world", "World"), ICON_FA_GLOBE)
	{
		auto main = std::make_shared<Category>(L("category.main", "Main"));
		auto iplsGroup = std::make_shared<Category>(L("category.ipls", "IPLs"));

		auto killPeds = std::make_shared<Group>(L("group.kill", "Kill"), 1);
		...
		auto deleteOpts = std::make_shared<Group>(L("group.delete", "Delete"), 2);
		...
		auto bringOpts = std::make_shared<Group>(L("group.bring", "Bring"), 1);
		...
		auto weatherOpts = std::make_shared<Group>(L("group.weather", "Weather"), 1);
		...
		auto timeGroup = std::make_shared<Group>(L("group.time", "Time"));
		...
		auto otherOpts = std::make_shared<Group>(L("group.other", "Other"), 1);
