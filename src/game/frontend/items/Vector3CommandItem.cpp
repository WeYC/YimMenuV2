...
namespace YimMenu
{
	...
	void Vector3CommandItem::Draw()
	{
		...
		{
			ImGui::SameLine();
			if (ImGui::Button(L("btn.current", "Current").c_str()))
				m_Command->SetState(Self::GetPed().GetPosition());
		}
		ImGui::SameLine();
		if (ImGui::Button(L("btn.saved", "Saved...").c_str()))
			ImGui::OpenPopup("##saved");

		if (ImGui::BeginPopup("##saved", ImGuiWindowFlags_NoTitleBar | ImGuiWindowFlags_NoMove | ImGuiWindowFlags_NoResize))
		{
			ImGui::Text("%s", L("saved_locations.instruction", "Click on a location to select it. Add more at Teleport > Saved").c_str());
			...
			ImGui::BeginGroup();
			ImGui::Text("%s", L("saved_locations.categories", "Categories").c_str());
			...
			ImGui::SameLine();
			ImGui::BeginGroup();
			ImGui::Text("%s", L("saved_locations.locations", "Locations").c_str());
